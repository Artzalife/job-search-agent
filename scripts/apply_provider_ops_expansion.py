#!/usr/bin/env python3
"""Apply Provider Operations expansion to data/companies.csv."""
...
VERIFIED_PATH = ROOT / "data" / "_provider_ops_verified.json"
SKIPPED_PATH = ROOT / "data" / "_provider_ops_skipped.json"
...
RECATEGORIZE_TO_PROVIDER_OPS = {
    # Sample list companies already in registry under other categories.
    "lumahealth",
    "cedar",
    "tebra",
    "innovaccer",
    ...
    "hint-health",
    "elation-health",
    # Additional provider-operations companies miscategorized elsewhere.
    "garner-health",
    "covera-health",
    "medrio",
    "medeanalytics",
...
MANUAL_REVIEW_EXTRA = [
    ...
    {
        "display_name": "Kyruus Health",
        "category": "Provider Operations",
        "reason": "No supported ATS board found via API probe",
        "slug_candidates": "kyruus, kyruushealth",
    },
    {
        "display_name": "ModMed",
        "category": "Provider Operations",
    ...
    },
    {
        ...
        "category": "Provider Operations",
        "reason": "No supported ATS board found via API probe",
        "slug_candidates": "awell, joinawell",
    },
    {
    ...
    {
        "display_name": "Blinq Health",
        "category": "Provider Operations",
        "reason": "Ambiguous slug match on lever/blinq (non-healthcare postings)",
        "slug_candidates": "blinq, blinqhealth",
    },
]
...
def main() -> None:
    ...
    recategorized = 0
    for row in rows:
        if row["company_id"] in RECATEGORIZE_TO_PROVIDER_OPS and row["category"] != "Provider Operations":
            row["category"] = "Provider Operations"
            recategorized += 1
    ...
    provider_ops_count = sum(1 for r in rows if r["category"] == "Provider Operations")
    print(
        f"Added {added} companies, recategorized {recategorized}, "
        f"Provider Operations total: {provider_ops_count}"
    )
